...
export interface IStorage {
  ...
  updateRouterCloudDdnsHostname(id: string, hostname: string): Promise<void>;
  updateLastSuccessfulConnectionMethod(id: string, method: 'native' | 'rest' | 'snmp'): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateRouterCloudDdnsHostname(id: string, hostname: string): Promise<void> {
    // Store Cloud DDNS hostname separately (for REST API HTTPS and display purposes only)
    // IP address remains unchanged and is used for reachability checks
    await db
      .update(routers)
      .set({ cloudDdnsHostname: hostname })
      .where(eq(routers.id, id));
  }

  async updateLastSuccessfulConnectionMethod(id: string, method: 'native' | 'rest' | 'snmp'): Promise<void> {
    await db
      .update(routers)
      .set({ lastSuccessfulConnectionMethod: method })
      .where(eq(routers.id, id));
  }
  ...
  async getMonitoredPorts(routerId: string): Promise<(MonitoredPort & { routerName?: string; portComment?: string })[]> {
    const result = await db
      .select({
        ...
        interfaceComment: monitoredPorts.interfaceComment,
        interfaceMacAddress: monitoredPorts.interfaceMacAddress,
        lastInterfaceUpdate: monitoredPorts.lastInterfaceUpdate,
