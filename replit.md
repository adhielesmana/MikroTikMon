...
- **Traffic Monitoring:** On-demand real-time (1-second WebSocket) and historical (60-second background polling for monitored ports), 2-year data retention, automatic interface metadata updates. **Poller dynamically reloads routers and ports from database every 60 seconds**, ensuring immediate awareness of database changes (e.g., after restore, new routers, new monitored ports). **Application automatically restarts after database restore** to refresh connections and reinitialize scheduler.
