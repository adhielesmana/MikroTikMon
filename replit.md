...
- **Alerting:** Configurable thresholds per port. Dual notification via Email and In-App Popup to owner AND all assigned users. Alert severity levels, acknowledgment workflow, and history tracking. **Smart alert de-duplication** prevents sending multiple alerts for the same ongoing issue - if an active unacknowledged alert exists for a specific interface/router, no duplicate alerts are created until the issue is resolved or acknowledged. Auto-acknowledgment when issues resolve, and independent port status monitoring. Dashboard shows only unacknowledged/active alerts. All alerts (router down, port down, traffic threshold) are sent to both router owner and all assigned users.
