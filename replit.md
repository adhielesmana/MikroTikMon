...
- **Smart Data-Only Restore:** New intelligent backup restore feature that extracts only data from old backups while preserving current database schema. Prevents schema conflicts when restoring backups created before schema changes. Restores core tables (users, routers, groups, monitored ports) and skips tables with schema mismatches. Enabled by default in Settings UI with toggle option for full restore.
...
- **Deployment Script**: `intelligent-deploy.sh` with smart behavior:
  ...
  - **Auto-creates directories**: `attached_assets/logos/`, `logs/`, `backups/` with proper ownership (UID 1000:1000 - nodejs user)
...
- **Database Backup & Restore:** Automated daily backups (2 AM), manual backup creation, intelligent data-only restore (extracts data from old backups while preserving current schema), full restore option, 30-day backup retention, admin-only access via Settings UI. Data-only restore prevents schema conflicts when restoring old backups by selectively importing core tables (users, routers, groups, monitored ports) while skipping tables with schema changes.
