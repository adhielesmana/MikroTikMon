...
- **Traffic Monitoring:** On-demand real-time (5-second WebSocket polling with animated gauge decay effect, 50-poll auto-pause) and historical (60-second background polling for monitored ports), 2-year data retention, automatic interface metadata updates. **Poller dynamically reloads routers and ports from database every 60 seconds**, ensuring immediate awareness of database changes (e.g., after restore, new routers, new monitored ports). **Application automatically restarts after database restore** to refresh connections and reinitialize scheduler. **Real-time gauges display smooth decay animation** from actual traffic values to zero over 5 seconds, creating active visual feedback between polls.
...
- **Performance:** HTTP caching with ETags, React Query optimization, memoized data processing, throttled chart updates, background scheduler, aggressive static asset caching, dynamic gauge scaling, **WebSocket payload optimization (98% reduction - sends only latest sample vs. 50-sample history)**, **alerts endpoint optimization with composite database indexes and split queries (target <300ms from 800-1000ms baseline)**.
