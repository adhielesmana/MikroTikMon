...
- **Performance:** Background scheduler (60s interval for monitored ports only), on-demand real-time polling (1s interval via WebSocket), indexed traffic data, efficient Drizzle ORM queries, real-time WebSocket updates, responsive UI, optimized alert queries, reduced server load by polling only when needed.
